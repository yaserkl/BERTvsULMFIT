...
def get_texts(df, n_lbls, lang='en'):
    ...
    #tok = Tokenizer(lang=lang).process_all(partition_by_cores(texts, n_cpus=cpu_count()))
...
def create_toks(dir_path, working_path, chunksize=24000, n_lbls=1, lang='en'):
    print(f'dir_path {dir_path} working_path {working_path} chunksize {chunksize} n_lbls {n_lbls} lang {lang}')
    try:
        spacy.load(lang)
    except OSError:
    ...
    df_trn = pd.read_csv(dir_path / 'train.csv', header=None, chunksize=chunksize, quoting=csv.QUOTE_NONE,
                  quotechar="", escapechar="\\")
    df_val = pd.read_csv(dir_path / 'val.csv', header=None, chunksize=chunksize, quoting=csv.QUOTE_NONE,
                  quotechar="", escapechar="\\")
