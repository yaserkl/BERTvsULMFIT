...
ROOT=$1
WIKI_DIR="${ROOT}/wiki"
DUMP_DIR="${WIKI_DIR}/wiki_dumps"
EXTR_DIR="${WIKI_DIR}/wiki_extr"
EXTR="wikiextractor"
mkdir -p "${ROOT}"
mkdir -p "${DUMP_DIR}"
mkdir -p "${EXTR_DIR}"
...
if [ ! -f "${DUMP_PATH}" ]; then
  ...
  case "$choice" in
    y|Y ) echo "Starting download..."; wget -c "https://dumps.wikimedia.org/""${LANG}""wiki/latest/""${DUMP_FILE}""" -P "${DUMP_DIR}";;
    n|N ) echo "Skipping download...";;
...
if [ ! -d "${EXTR_PATH}" ]; then
  ...
  case "$choice" in
    y|Y ) echo "Extracting ${DUMP_PATH} to ${EXTR_PATH}...";python wikiextractor/WikiExtractor.py -s --json -o "${EXTR_PATH}" "${DUMP_PATH}";;
    n|N ) echo "Skipping extraction";;
    * ) echo "Invalid answer";exit 1;;
  esac
else
...
case "$choice" in
y|Y ) echo "Merging articles from ${EXTR_PATH} to ${OUT_PATH}...";python merge_wiki.py -i "${EXTR_PATH}" -o "${OUT_PATH}";;
n|N ) echo "Skipping the Wikipedia merge";exit 1;;
* ) echo "Invalid answer";exit 1;;
esac